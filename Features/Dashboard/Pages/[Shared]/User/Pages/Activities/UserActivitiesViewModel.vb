...
Public Class UserActivitiesViewModel
    ...
    Private Async Sub Load()
        Try
            ...
            _activities = Await Task.Run(Function() _activityService.GetUserActivity(_userDetails)).ConfigureAwait(True)
    ...
    Public Async Sub OnNavigatedTo(navigationContext As NavigationContext) Implements IRegionAware.OnNavigatedTo
        Try
            ...
            End If
        Catch ex As Exception
            Debug.WriteLine($"[UserActivitiesViewModel] OnNavigatedTo Error: {ex.Message}")
            PopUp.Information("Failed", "An error occurred while navigating to the user activities view.")
        Finally
            Loading.Hide()
        End Try
    End Sub
