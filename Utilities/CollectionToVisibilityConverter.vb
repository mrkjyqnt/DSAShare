Imports System.Globalization

Public Class CollectionToVisibilityConverter
    Implements IValueConverter

    Public Function Convert(value As Object, targetType As Type, parameter As Object, culture As CultureInfo) As Object Implements IValueConverter.Convert
        If value Is Nothing Then Return Visibility.Visible
        
        ' Handle both ICollection and IEnumerable
        Dim count As Integer = 0
        If TypeOf value Is ICollection Then
            count = DirectCast(value, ICollection).Count
        ElseIf TypeOf value Is IEnumerable Then
            count = DirectCast(value, IEnumerable).OfType(Of Object)().Count()
        End If
        
        Return If(count > 0, Visibility.Collapsed, Visibility.Visible)
    End Function

    Public Function ConvertBack(value As Object, targetType As Type, parameter As Object, culture As CultureInfo) As Object Implements IValueConverter.ConvertBack
        Throw New NotImplementedException()
    End Function
End Class